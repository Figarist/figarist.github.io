...
- **Language Preference Persistence:** A click handler in `header.html` saves the user's manual language choice to `localStorage` (`preferred_lang`), so the auto-redirect respects it on subsequent visits.
...
- **Language Switcher:** Generated in `_includes/header.html` via a `{% for lang in site.languages %}` loop. Uses `{% static_href %}` to bypass Polyglot's aggressive URL rewriting.
...
- **Automated SEO Plugins:**
  - `jekyll-seo-tag` — generates `<title>`, Open Graph, Twitter Cards, JSON-LD `Person`/`WebSite`/`BlogPosting` schemas, and `<link rel="canonical">` automatically from YAML front matter (`title`, `description`, `image:`).
  - `jekyll-sitemap` — generates `sitemap.xml` automatically.
  - `jekyll-feed` — generates `feed.xml` (RSS) automatically.
